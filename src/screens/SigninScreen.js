// TODO formik isn't resetting textfields to active if there was an error previously...
// TODO signup doesn't use formik but does reset after an error so maybe consider removing formik
...
export default function SigninScreen() {
  const { state, signin, clearErrorMessage, resetPassword } = useContext(
    AuthContext
  );
  const authError = state.errorMessage;
  const resetPasswordDone = state.emailSent;
  ...
  useEffect(() => {
    resetPasswordDone ? setEmailSent(true) : setEmailSent(false);
  }, [resetPasswordDone]);
  ...
  // TODO get ConfirmationSnackbar to show after email is sent and modal closes
    ...
    <Container component="main" maxWidth="xs">
      ...
      {useError && (
        <ErrorSnackbar
          ...
          handleClearError={handleErrorMessage}
        />
      )}
